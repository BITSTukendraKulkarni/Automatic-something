...
def start_workflow(
    ...
    """Start the CrewAI workflow. Returns (status_msg, log_output)."""
    ...
    return f"🚀 {msg}", "Workflow starting..."
...
"""
                ...
                start_btn.click(
                    ...
                    outputs=[workflow_status, gr.Textbox(visible=False)],
                )
            ...
            with gr.Tab("📋 Live Log"):
                ...
                # Auto-refresh via Gradio's built-in timer (Gradio 4.x)
                # Uses every= parameter on Timer if available
                try:
                    timer = gr.Timer(value=3)
                    timer.tick(fn=refresh_log, outputs=[log_output])
                    auto_refresh.change(
                        fn=lambda v: gr.Timer(active=v),
                        inputs=[auto_refresh],
                        outputs=[timer],
                    )
                except AttributeError:
                    # Fallback for older Gradio — manual refresh only
                    pass
